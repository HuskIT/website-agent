/**
 * API Routes Contract for Sandbox Operations
 ...
 * Actual Implementation: app/lib/sandbox/schemas.ts (canonical source)
 *
 * Routes implemented in app/routes/:
 * - api.sandbox.create.ts
 * - api.sandbox.reconnect.ts
 * - api.sandbox.files.ts
 * - api.sandbox.files.$path.ts
 * - api.sandbox.command.ts
 * - api.sandbox.snapshot.ts
 * - api.sandbox.snapshot.$id.restore.ts
 * - api.sandbox.status.ts
 * - api.sandbox.extend.ts
 * - api.sandbox.stop.ts
 * - api.user.sandbox-preference.ts
 */
...
export const CreateSandboxRequestSchema = z.object({
  ...
  ports: z.array(z.number().int().min(1).max(65535)).default([3000, 5173, 8080]),
  timeout: z.number().int().min(60000).max(18000000).default(300000), // 1min - 5hr, default 5min
});
...
export const ReconnectSandboxResponseSchema = z.object({
  success: z.boolean(),
  status: z.enum(['running', 'stopped', 'failed', 'not_found']),
...
export const GetStatusResponseSchema = z.object({
  ...
  timeout: z.number(), // Remaining timeout in ms
  createdAt: z.string().datetime(),
  previewUrls: z.record(z.number(), z.string().url()),
});
