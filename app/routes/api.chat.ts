...
/**
 * Convert UIMessage[] (AI SDK v6 parts-based format) to LegacyMessage[]
 * (content-based format used by createSummary and selectContext).
 */
function uiMessagesToLegacy(messages: UIMessage[]): LegacyMessage[] {
 ...
 */
function estimateMessagesTokens(messages: LegacyMessage[]): number {
...
function truncateMessagesIfNeeded(
...
): { messages: LegacyMessage[]; truncated: boolean; originalTokens: number; finalTokens: number } {
   ...
   */
  const truncatedMessages: LegacyMessage[] = [];
  let currentTokens = 0;
...
async function chatAction({ context, request }: ActionFunctionArgs, session: { user?: { id?: string } }) {
  ...
  const userMessageContent = lastUserMessage ? getTextFromUIMessage(lastUserMessage) : '';
  ...
  try {
    ...
    const { model: extractedModel, provider: extractedProviderName } = lastUserMsg
      ? extractPropertiesFromMessage({
          role: lastUserMsg.role as 'user',
          content: getTextFromUIMessage(lastUserMsg),
        })
      : { model: '', provider: '' };
    ...
    const uiMessageStream = createUIMessageStream({
      async execute({ writer }) {
        ...
        const options: StreamingOptions = {
          ...
          onStepFinish: async ({ toolCalls, toolResults }) => {
            ...
            toolCalls.forEach((toolCall) => {
              mcpService.processToolCall(toolCall as unknown as import('~/lib/services/mcpService').ToolCall, writer);
            });
            ...
            const finalizeResult = toolResults?.find(
              (result) =>
                result.toolName === 'finalizeCollection' &&
                (result.output as { hasPendingInjection?: boolean })?.hasPendingInjection,
            );

            if (finalizeResult) {
              const { sessionId, generation } = finalizeResult.output as {
                sessionId: string;
                generation: { templateName?: string; themeId?: string };
              };
          ...
          onFinish: async ({ text: content, finishReason, usage }) => {
            ...
            const { model, provider } = extractPropertiesFromMessage(lastUserMessage as unknown as LegacyMessage);
         ...
         */
        const legacyMessages = uiMessagesToLegacy(processedMessages);
        const tokenGuardResult = truncateMessagesIfNeeded(legacyMessages);
    ...
    const errorResponse = {
      ...
      isRetryable: (error as { isRetryable?: boolean }).isRetryable !== false, // Default to retryable unless explicitly false
