    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (!supabaseUrl || !supabaseKey) {
          ...
          setTimeout(() => {
            navigate('/?error=missing_credentials', { replace: true });
          }, 2000);
        ...
        // ✅ Store token expiry time (Supabase tokens typically expire in 1 hour)
        ...
        try {
          ...
          if (!sessionResponse.ok) {
            console.warn('Session verification failed, but proceeding with local auth');
          } else {
            const sessionData = await sessionResponse.json();
            console.log('Session verified:', sessionData);
          }
        } catch (sessionError) {
          console.warn('Could not verify session with backend:', sessionError);
        }
