...
if (savedCredentials && !initialState.credentials) {
  try {
    ...
    logger.error('Failed to parse saved credentials', { error: e });
  }
}
...
export function initializeSupabaseConnection() {
  ...
  if (envToken && !supabaseConnection.get().token) {
    ...
    fetchSupabaseStats(envToken).catch((error) => {
      logger.error('Failed to fetch Supabase stats on init', { error });
    });
  }
}
